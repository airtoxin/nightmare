...
function Nightmare(options) {
  ...
  this.queue((done) => {
    ...
    function handlerFor (name) { return function () { log(name, JSON.stringify(Array.prototype.slice.call(arguments))); }; }
    this.child.on('did-finish-load', handlerFor('did-finish-load'));
    this.child.on('did-fail-load', handlerFor('did-fail-load'));
    this.child.on('did-fail-provisional-load', handlerFor('did-fail-provisional-load'));
    this.child.on('did-frame-finish-load', handlerFor('did-frame-finish-load'));
    this.child.on('did-start-loading', handlerFor('did-start-loading'));
    this.child.on('did-stop-loading', handlerFor('did-stop-loading'));
    this.child.on('did-get-response-details', handlerFor('did-get-response-details'));
    this.child.on('did-get-redirect-request', handlerFor('did-get-redirect-request'));
    this.child.on('dom-ready', handlerFor('dom-ready'));
    this.child.on('page-favicon-updated', handlerFor('page-favicon-updated'));
    this.child.on('new-window', handlerFor('new-window'));
    this.child.on('will-navigate', handlerFor('will-navigate'));
    this.child.on('crashed', handlerFor('crashed'));
    this.child.on('plugin-crashed', handlerFor('plugin-crashed'));
    this.child.on('destroyed', handlerFor('destroyed'));
    this.child.on('media-started-playing', handlerFor('media-started-playing'));
    this.child.on('media-paused', handlerFor('media-paused'));
  ...
  keys(Nightmare.childActions).forEach(function(key){
